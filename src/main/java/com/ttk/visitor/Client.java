 ...
 * @Description 测试访问者模式
 *      1）定义：
 *          - 封装一些作用于某种数据结构中的各元素的操作，它可以在不改变这个数据结构的前提下定义作用于这
 *            些元素的新的操作。
 *      2）访问者模式包含以下主要角色:
 ...
 *          - 抽象元素（Element）角色：定义了一个接受访问者的方法（ accept ），其意义是指，每一个元
 *            素都要可以被访问者访问。
 ...
 *          - 分离无关行为。通过访问者来分离无关的行为，把相关的行为封装在一起，构成一个访问者，这样
 *            每一个访问者的功能都比较单一。
 *      4）缺点：
 *          - 对象结构变化很困难。在访问者模式中，每增加一个新的元素类，都要在每一个具体访问者类中增加
 *            相应的具体操作，这违背了“开闭原则”。
