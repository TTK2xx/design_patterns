 ...
 *            的次序，也可动态地新增或者删除责任。
 *          - 责任链简化了对象之间的连接。一个对象只需保持一个指向其后继者的引用，不需保持其他所有处
 *            理者的引用，这避免了使用众多的 if 或者 if···else 语句。
 *          - 责任分担。每个类只需要处理自己该处理的工作，不能处理的传递给下一个对象完成，明确各类的
 *            责任范围，符合类的单一职责原则。
 *      4）缺点：
 *          - 不能保证每个请求一定被处理。由于一个请求没有明确的接收者，所以不能保证它一定会被处理，
 *            该请求可能一直传到链的末端都得不到处理。
 *          - 对比较长的职责链，请求的处理可能涉及多个处理对象，系统性能将受到一定影响。
 *          - 职责链建立的合理性要靠客户端来保证，增加了客户端的复杂性，可能会由于职责链的错误设置而
 *            导致系统出错，如可能会造成循环调用。
 *      5）适用场景：
 *          -
 *      6）案例情景：
 *          - 现需要开发一个请假流程控制系统。请假一天以下的假只需要小组长同意即可；请假1天到3天的假还需
 *            要部门经理同意；请求3天到7天还需要总经理同意才行。
 * @Version 1.0
 */
public class Client {
